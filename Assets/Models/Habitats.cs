...
public class Habitats
{
    ...
    // CLIMATE CONSTANTS
    private const int ARTIC_MIN_COLD_DAYS = 40;
    private const int ARTIC_MAX_HOT_DAYS = 10;
    private const int TROP_MIN_HOT_DAYS = 40;
    private const int TROP_MAX_COLD_DAYS = 10;
    private const double VERY_DRY_MAX_RAIN = 10.0;
    private const double DRY_MAX_RAIN = 25.0;
    private const double MODERATE_MAX_RAIN = 40.0;
    ...
    {
        string idealsString = MyJsonFileInteractor.loadDataFileToString("Ideals");
        if (ideals == null)
        {
    ...
    public void growHabitats(int[] temps, double totalPrecipitation, double avgRiverLevel, bool isIceSheet)
    {
    ...
    public Dictionary<string, double[]> getYearOfGatherables(int[] dailyTemps, double[] dailyRain, double[] dailyVolume)
    {
        if (crops == null)
        {
    ...
    private int getFavoredTempRegion(int coldDays, int hotDays)
    {
        ...
        int multiplier = 1;
        // tropical & artic
        if (coldDays >= ARTIC_MIN_COLD_DAYS && hotDays <= ARTIC_MAX_HOT_DAYS)
        {
            multiplier = 0;
        }
        else if (hotDays >= TROP_MIN_HOT_DAYS && coldDays <= TROP_MAX_COLD_DAYS)
        {
            multiplier = 2;
        }
    ...
    private int getFavoredPrecipLevel(double functionalPrecipitation)
    {
        ...
        {
            case double wet when (wet >= VERY_DRY_MAX_RAIN && wet <DRY_MAX_RAIN):
                // Dry
                addifier = 1;
                break;
            case double wet when (wet >= DRY_MAX_RAIN && wet <MODERATE_MAX_RAIN):
                // Moderate
                addifier = 2;
                break;
            case double wet when (wet >= MODERATE_MAX_RAIN):
                // Wet
                addifier = 3;
                break;
        }
