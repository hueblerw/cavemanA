...
namespace CavemanLand.Models.GenericModels
{
    public class Plant
    {
        ...
        public double[] getYearOfCrop(double habitatPercentage, int[] dailyTemps, double[] dailyRain, double[] dailyVolume)
        {
            ...
            {
                if (!dayTempAllowCrop(day, dailyTemps))
                {
                    lastInvalidDay = day;
                }
                if (day - lastInvalidDay >= growthPeriod - 1 && dayRainAllowCrop(day, dailyRain, dailyVolume, out rainSum, out percentGrowable))
                {
                    double cropMultiplier = (1.0 / ((80 - growthPeriod) * 100.0)) * 400.0 * habitatPercentage;
                    currentCrop[day] = Math.Round(calculateCropQuality(day, rainSum, dailyTemps) * cropMultiplier * unitPerHarvest * percentGrowable, World.ROUND_TO);
